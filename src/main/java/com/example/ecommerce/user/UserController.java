...
public class UserController {
    ...
    public ResponseEntity<Object> postUser(@RequestBody User user) {
        try {
            Status request = userService.verify(user);
            if (request == Status.EMAIL_EXIST) {
                return Response.generateResponse(HttpStatus.CONFLICT, "email already exist", null);
            }
            ...
            return Response.generateResponse(HttpStatus.CREATED, "successfully add data", user);
